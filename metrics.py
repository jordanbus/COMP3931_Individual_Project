...
class MulticlassMetrics:
    ...
    def dice_loss_wrapper(class_weights, smooth=1e-6):
        def dice_loss(y_true, y_pred):
            ...
            loss = 0
            for i in range(channels):
                loss += class_weights[i] * MulticlassMetrics._dice_loss_for_label(y_true, y_pred, i, smooth)
            return loss
        return dice_loss
    ...
    def dice_coef_necrotic(y_true, y_pred, smooth=1e-6):
        return 1 - MulticlassMetrics._dice_loss_for_label(y_true, y_pred, 1, smooth)

    def dice_coef_edema(y_true, y_pred, smooth=1e-6):
        return 1 - MulticlassMetrics._dice_loss_for_label(y_true, y_pred, 2, smooth)

    def dice_coef_enhancing(y_true, y_pred, smooth=1e-6):
        return 1 - MulticlassMetrics._dice_loss_for_label(y_true, y_pred, 3, smooth)
    ...
    def combined_loss_wrapper(class_weights, alpha=0.5, smooth=1e-5):
        def combined_loss(y_true, y_pred):
            ...
            bce_fn = MulticlassMetrics.binary_cross_entropy_per_channel_wrapper(class_weights)
            bce = bce_fn(y_true, y_pred)

            # Calculate Dice loss
            dl_fn = MulticlassMetrics.dice_loss_wrapper(class_weights)
...
class BinaryMetrics:
    ...
    def combined_loss_wrapper(alpha=0.5, smooth=1e-5):
        def combined_loss(y_true, y_pred):
            ...
            # Calculate Dice loss
            dl = BinaryMetrics.dice_loss(y_true, y_pred)
