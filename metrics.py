...
class MultilabelMetrics:
    ...
    # Per class evaluation of binary cross entropy
    def binary_cross_entropy_per_channel(self,y_true, y_pred):
    ...
    # - Specificity
    def specificity(self,y_true, y_pred, smooth=1e-6):
    ...
    # - Sensitivity
    def sensitivity(self,y_true, y_pred, smooth=1e-6):
    ...
    def _dice_loss_for_label(self,y_true, y_pred, label, smooth=1e-6):
    ...
    def dice_loss(self,y_true, y_pred, smooth=1e-6):
        ...
        loss = 0
        for i in range(channels):
            loss += class_weights[i] *  self._dice_loss_for_label(y_true, y_pred, i, smooth)
        return loss

 
    # - Dice coefficient index for each class
    def dice_coef_necrotic(self,y_true, y_pred, smooth=1e-6):
        return 1 - self._dice_loss_for_label(y_true, y_pred, 1, smooth)

    def dice_coef_edema(self,y_true, y_pred, smooth=1e-6):
        return 1 - self._dice_loss_for_label(y_true, y_pred, 2, smooth)


    def dice_coef_enhancing(self,y_true, y_pred, smooth=1e-6):
        return 1 - self._dice_loss_for_label(y_true, y_pred, 3, smooth)

    # Combined loss function that combines Binary cross entropy and Dice loss, weighted with alpha
    def combined_loss(self,y_true, y_pred, alpha=0.5, smooth=1e-5):
        # Calculate binary cross entropy
        bce = self.binary_cross_entropy_per_channel(y_true, y_pred)
